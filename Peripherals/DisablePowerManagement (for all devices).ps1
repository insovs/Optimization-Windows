...
if ($wake) { foreach ($d in $wake) { Set-CimInstance -InputObject $d -Property @{Enable = $false}; Write-Host "[OK] $($d.InstanceName)" } } else { Write-Host "[SKIP] No other devices with wake capability enabled." }

$power = Get-CimInstance -Query 'SELECT InstanceName FROM MSPower_DeviceEnable WHERE (Enable = True)' -Namespace 'root\WMI'
if ($power) { foreach ($d in $power) { Set-CimInstance -InputObject $d -Property @{Enable = $false}; Write-Host "[OK] $($d.InstanceName)" } } else { Write-Host "[SKIP] No other devices with power management enabled." }
